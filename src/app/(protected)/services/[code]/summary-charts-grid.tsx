...
export function SummaryChartsGrid({ summaryData, maxTrendTotal }: SummaryChartsGridProps) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-5 shadow-xs flex flex-col justify-between min-h-[320px]">
        ...
        <div className="py-6 flex-1 flex flex-col justify-end">
          {summaryData?.emissionsTrend && summaryData.emissionsTrend.length > 0 ? (
            <div className="h-48 w-full flex items-end justify-around gap-4 px-4 border-b border-l border-border relative">
              {summaryData.emissionsTrend.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end relative">
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 left-1/2 -translate-x-1/2 bg-foreground text-background text-[10px] p-2 rounded-lg font-mono pointer-events-none whitespace-nowrap z-20 shadow-lg border border-border">
                      <p className="font-bold border-b border-border pb-1 mb-1">{item.period}</p>
                      ...
                      <p className="font-extrabold text-background pt-1 border-t border-border">Total: {item.total} t CO₂-e</p>
                    </div>

                    <div
                      className="w-12 bg-muted rounded-t-lg overflow-hidden flex flex-col justify-end group-hover:ring-2 group-hover:ring-primary/50 transition-all cursor-pointer"
            ...
            <div className="h-44 flex flex-col items-center justify-center text-center p-4">
              ...
              <p className="text-xs font-semibold text-muted-foreground">No trend data found in database</p>
              <p className="text-[11px] text-muted-foreground/60">Add inventory entries to render carbon trend charts.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border p-5 shadow-xs flex flex-col justify-between min-h-[320px]">
        <div className="flex items-center justify-between pb-3 border-b border-border">
          <div className="flex items-center gap-2">
        ...
        <div className="py-3 flex-1 flex flex-col justify-start space-y-3.5 overflow-y-auto max-h-[220px] pr-1">
          {summaryData?.emissionsByFacility && summaryData.emissionsByFacility.length > 0 ? (
            summaryData.emissionsByFacility.map((fac, i) => (
              <div key={i} className="space-y-1">
                <div className="flex items-center justify-between text-xs font-bold text-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-extrabold text-foreground">{fac.emission} t</span>
                    <span className="text-[10px] text-muted-foreground">({fac.percentage}%)</span>
                  </div>
                </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-xs text-muted-foreground font-medium">No facility emissions logged yet.</p>
            </div>
          )}
        </div>
      </div>
