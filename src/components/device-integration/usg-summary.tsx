...
export function UsgConfigurationSummary({
  onConfigure,
    ...
    const usgName = form.watch("usgName");
    ...
    useEffect(() => {
        const fetchRoomDetails = async () => {
          try {
            ...
            if (response.success) {
              const { usgName, usgDescription, usgIp, usgPort, usgAeTitle, roomId, usgId } = response.data;
              form.reset({
                usgName,
                usgDescription,
                usgIp,
            ...
            <div className="flex flex-col gap-4 bg-background-outer rounded-lg p-4">
              ...
              <div className="flex flex-row justify-between items-end gap-4">
                <div className="flex flex-row gap-4">
                  <FormInput
                    className="w-[22.5rem] cursor-not-allowed disabled:opacity-100 disabled:text-text-primary"
                    name="usgIp"
                    label="IP Address"
                    vertical
                    disabled
                  />
                  <FormInput
                    className="w-[10rem] cursor-not-allowed disabled:opacity-100 disabled:text-text-primary"
                    name="usgPort"
                    label="Port"
                    vertical
                    disabled
                  />
                  <FormInput
                    className="w-[11.25rem] cursor-not-allowed disabled:opacity-100 disabled:text-text-primary"
                    name="usgAeTitle"
                    label="AE Title"
                    vertical
                    disabled
