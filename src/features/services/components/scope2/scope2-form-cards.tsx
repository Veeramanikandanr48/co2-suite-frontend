    ...
    <div className={`grid grid-cols-1 xl:grid-cols-3 gap-4 ${isNotRelevant ? 'opacity-40 pointer-events-none select-none' : ''}`}>
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-xs flex flex-col justify-between space-y-3">
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-lg p-2.5 text-[11px] text-primary font-medium">
          Active EF Value: <span className="font-bold text-primary">{currentMatchingEF?.factor ?? 0.42}</span> {currentMatchingEF?.unit || 'kg CO₂e/kWh'}
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-xs space-y-3">
        ...
        <div className="space-y-3 pt-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="block text-[11px] font-semibold text-muted-foreground mb-1">Unit</label>
              <select
                value={unit}
                onChange={(e) => setUnit(e.target.value)}
                className="w-full bg-muted border border-border rounded-lg px-2 py-1.5 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              >
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-xs flex flex-col justify-between space-y-3">
        <div>
          ...
          <div className="mt-3 space-y-2.5">
            <div>
              <label className="block text-[11px] font-semibold text-muted-foreground mb-1">Facility Site</label>
              <select
                value={facility}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[11px] font-semibold text-muted-foreground mb-1">Approval Status</label>
                <select
